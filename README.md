...
- 菜单栏/托盘：状态 / 暂停热键 / **设置…（macOS 原生设置窗口：热键、录音方式、AI 润色一站配好）** / 快捷设置 / 历史 / 统计 / 日志；热词、替换表、场景感知等高级项走「编辑完整配置(JSON)」

图标状态（单色模板图标，自动适配深浅色菜单栏）：声浪＝待机 → **红色声浪**＝录音中（旁显已录秒数）→ ···＝转写中 → ✦＝润色中 → 声浪变淡＝已暂停
