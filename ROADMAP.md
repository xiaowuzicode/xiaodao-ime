...
- Windows 原生设置窗口（对齐 macOS 的「设置…」面板；现阶段 Windows 仍编辑 settings.json）。
- 权限自检误报：CGPreflightListenEventAccess 在已授权情况下可能返回 false（实测键盘事件能收到但状态行报「缺权限」）；应改为功能性判定——收到首个键盘事件后自动清除「缺权限」状态。
